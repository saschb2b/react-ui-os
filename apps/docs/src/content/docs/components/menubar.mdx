...
Related `chrome` tokens: `menuBarClock` (`"right"` / `"center"`), `menuBarBrand` (`true` / `false`, whether the bar carries the brand button), and `quickSettings` (opens the [Quick Settings](/components/quicksettings/) popover from the status cluster).
