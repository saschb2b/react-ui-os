...
| `files`        | The files in `dir` to copy; subpaths like `ui/Panel.tsx` work |
...
The CLI installs from any registry, not just the built-in one. Point `--registry`
...
You do not need to add your app to this repository. Build your registry into a
single self-contained file (every app's source inlined) and host it anywhere
that serves files over HTTP: GitHub Pages, a CDN, an object store.
...
A remote registry must be the built (inlined) form, because the CLI cannot read
your repository's files over HTTP. The `build` command produces it.

## Propose for the official registry

The built-in registry is curated and intentionally small. If an app is broadly
