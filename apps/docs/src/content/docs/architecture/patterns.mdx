...
The library exposes four points at which a consumer hands the system something to render or surface.
...
All four are **declarative**. You hand the library a description; it owns the rendering, the keyboard nav, the focus, the dismissal. Re-registering the same id replaces the previous record so a host component can update without churn.
