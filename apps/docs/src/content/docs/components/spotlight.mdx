...
<LivePreview src="?demo=spotlight" label="The launcher in its Spotlight register" height="420px" />
...
The default theme uses `"spotlight"`, Ubuntu uses `"grid"`, and Redmond uses `"menu"`. The arrow keys navigate every register (the grid and menu add left/right); Enter activates; Escape dismisses and restores focus.
