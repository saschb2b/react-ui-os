...
The column mirrors the macOS desktop selection model (the same one [FileExplorer](/components/fileexplorer/) uses) and is a single ARIA listbox (`role="listbox"` + `role="option"` with `aria-selected`), so the whole column is one tab stop and assistive tech announces the active icon.
