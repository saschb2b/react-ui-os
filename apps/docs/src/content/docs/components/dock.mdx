...
Each tile's size is a pure function of the cursor's distance to that tile's resting center, anchored to the dock center, so there is no layout-measurement feedback. The values follow the common macOS approximation (a 110 px radius and a 2.25x peak), tuned here to a gentler 1.8x for a 56 px base tile.
