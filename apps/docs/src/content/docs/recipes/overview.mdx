...
import { useWindowManager, SPOTLIGHT_OPEN_EVENT } from "@react-ui-os/desktop";
...
Pass a custom [`StorageAdapter`](/api/storageadapter/) that writes to your backend instead of `localStorage`. The window manager treats it the same way.
...
const remoteStorage: StorageAdapter = {
  get: async (key) => {
    const res = await fetch(`/api/prefs/${key}`);
    return res.ok ? res.text() : null;
  },
  set: (key, value) => {
    void fetch(`/api/prefs/${key}`, { method: "PUT", body: value });
  },
  remove: (key) => {
    void fetch(`/api/prefs/${key}`, { method: "DELETE" });
  },
  subscribe: (listener) => {
    const es = new EventSource("/api/prefs/stream");
    es.onmessage = (e) => listener(e.data);
    return () => es.close();
  },
};
