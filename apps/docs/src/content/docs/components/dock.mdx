...
| `"top"`    | Horizontal run at the top edge; magnified icons grow downward.                         |
| `"left"`   | Vertical rail centered on the left edge; magnified icons grow inward.                  |
| `"right"`  | Vertical rail centered on the right edge; magnified icons grow inward.                 |
| `"hidden"` | Returns `null`. Apps are reachable via Spotlight, keyboard shortcuts, or `openWindow`. |

All four edges follow the movable Windows taskbar (Insider builds, May 2026). The Start-style launcher, notification surfaces, and desktop icons track the chosen edge so nothing overlaps the bar.
...
The work area reserves the dock's thickness either way, so maximized windows stop above a taskbar just as they stop above a floating dock. The [Windows theme](/themes/overview/) ships `"bar"`; every other theme uses `"floating"`.

`chrome.dockSmallButtons` is the Windows "Show smaller taskbar buttons" control. `"always"` compacts the bar to its small register (48px to 32px on the Windows theme, with 16px icons) and gives the space back to the work area; `"when-full"` (the Windows default) keeps the bar height and drops to small buttons only while the icon run would overflow; `"never"` keeps full size.
...
The peak is the `theme.motion.dockMagnification` token. Set it to `1` to turn the fisheye off entirely, the register a Windows taskbar or GNOME dash uses; the [Windows theme](/themes/overview/) ships it that way. The hover label still appears, only the growth stops.
