...
const apps: App[] = [
  { id: "notes", name: "Notes", content: () => <h1>Notes</h1> },
];

<Desktop apps={apps} theme={defaultTheme} brand="acme" />;
```
...
| `theme`    | `OsTheme`             | **required**     | The active theme. Pass `defaultTheme` or call `createMintablesTheme(...)`.                                                                                                   |
| `brand`    | `string`              | `"react-ui-os"`  | Label shown on the left of the menu bar.                                                                                                                                     |
...
### With a custom brand and the Mintables theme
